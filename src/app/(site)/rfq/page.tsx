    ...
    <div>
      ...
      <div className="container-site section">
        <div style={{ display: 'grid', gridTemplateColumns: '1fr', gap: '2.5rem', alignItems: 'start' }}>
          <div style={{ background: '#fff', border: '1px solid #E2E8F0', borderRadius: '0.75rem', padding: '2rem' }}>
            <RFQForm />
          </div>

          <div style={{ display: 'flex', flexDirection: 'column', gap: '1.25rem' }}>
            <div style={{ background: '#0F172A', borderRadius: '0.75rem', padding: '1.5rem', color: '#94a3b8' }}>
              <h3 style={{ color: '#fff', fontWeight: 700, fontSize: '1rem', marginBottom: '1rem' }}>What happens next?</h3>
              {[
            ...
            <div style={{ background: '#f0fdf4', border: '1px solid #bbf7d0', borderRadius: '0.75rem', padding: '1.5rem', textAlign: 'center' }}>
              <h3 style={{ fontWeight: 700, color: '#0F172A', marginBottom: '0.5rem' }}>Prefer WhatsApp?</h3>
              <p style={{ fontSize: '0.875rem', color: '#64748B', marginBottom: '1rem' }}>Chat directly with our team for faster response.</p>
              <a href={`https://wa.me/${WHATSAPP}?text=${encodeURIComponent('Hi, I want a wholesale quote for mobile devices.')}`} target="_blank" rel="noopener noreferrer" className="btn btn-whatsapp" style={{ width: '100%', justifyContent: 'center' }}>
                <MessageCircle size={16} /> Chat on WhatsApp
              </a>
            </div>

            <div style={{ background: '#fff', border: '1px solid #E2E8F0', borderRadius: '0.75rem', padding: '1.5rem', display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: '1rem', textAlign: 'center' }}>
              {[{ value: '500+', label: 'Products' }, { value: '< 2h', label: 'Response' }, { value: '50+', label: 'Countries' }].map(s => (
                <div key={s.label}>
