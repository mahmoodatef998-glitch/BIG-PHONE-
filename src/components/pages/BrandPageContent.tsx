...
export default function BrandPageContent({ brand, products }: { brand: Brand; products: Product[] }) {
  const { t } = useLanguage();
  const [bg1, bg2] = BRAND_GRADIENT[brand.slug] ?? ['#1A2332', '#2D3748'];
    ...
    <div style={{ background: '#F8FAFC', minHeight: '100vh' }}>
      <div style={{ background: `linear-gradient(135deg, ${bg1}, ${bg2})`, padding: '2.5rem 0 2rem' }}>
        <div className="container-site">
          ...
          >
            <ChevronLeft size={14} /> {t.brands.backToInventory}
          </Link>
          <div style={{ display: 'flex', alignItems: 'center', gap: '1rem' }}>
            ...
            <div>
              ...
              <p style={{ color: 'rgba(255,255,255,0.6)', fontSize: '0.875rem', margin: '0.25rem 0 0' }}>
                {fmt(t.brands.productsInStock, { n: products.length })}
              </p>
            </div>
          </div>
          {brand.description && (
            <p style={{ color: 'rgba(255,255,255,0.7)', fontSize: '0.9375rem', marginTop: '1.25rem', maxWidth: '480px', lineHeight: 1.6 }}>
              {brand.description}
            </p>
          )}
        </div>
      ...
      <div style={{ background: '#fff', borderBottom: '1px solid #DDE3EA', padding: '0.75rem 0' }}>
        <div className="container-site">
          <nav style={{ fontSize: '0.75rem', color: '#94a3b8', display: 'flex', gap: '0.25rem', alignItems: 'center' }}>
            ...
            <Link href="/inventory" style={{ color: '#0066FF', textDecoration: 'none' }}>{t.inventory.breadcrumb}</Link>
            <span style={{ color: '#CBD5E1' }}>›</span>
        ...
        {products.length === 0 ? (
          <div style={{ textAlign: 'center', padding: '4rem 1rem', background: '#fff', borderRadius: '12px', border: '1px solid #DDE3EA' }}>
            <p style={{ color: '#64748B', marginBottom: '1rem' }}>
              {fmt(t.brands.noProductsFor, { brand: brand.name })}
            </p>
            <Link href="/inventory" style={{ color: '#0066FF', textDecoration: 'none', fontWeight: 600, fontSize: '0.875rem' }}>
              {t.brands.browseAll}
            </Link>
          </div>
        ) : (
